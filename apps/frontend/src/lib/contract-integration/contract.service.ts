...
export class ContractService {
  ...
  async getLoanMetadata(tokenId: string) {
    try {
      ...
      if (!publicClient || typeof publicClient.readContract !== 'function') {
        console.warn('Public client not available or missing readContract method');
        return null;
      }
  ...
  async hasMinterRole(address: string): Promise<boolean> {
    try {
      ...
      if (import.meta.env.DEV || import.meta.env.MODE === 'development') {
        console.log('Development mode detected, mocking minter role check to return true');
        return true;
      }
      
      const publicClient = getPublicClient({ chainId: this.config.chainId } as any);
      
      // Verify that publicClient exists and has the readContract method
      if (!publicClient || typeof publicClient.readContract !== 'function') {
        console.warn('Public client not available or missing readContract method, defaulting to false');
        return false;
      }
  ...
  async getActiveLoans(): Promise<string[]> {
    try {
      ...
      // Verify that publicClient exists and has the readContract method
      if (!publicClient || typeof publicClient.readContract !== 'function') {
        console.warn('Public client not available or missing readContract method, returning empty array');
        return [];
      }
